...
struct phy_meson_g12a_mipi_dphy_priv {
	struct device				*dev;
...
static const struct regmap_config phy_meson_g12a_mipi_dphy_regmap_conf = {
	.reg_bits = 8,
	.val_bits = 32,
	.reg_stride = 4,
...
static int phy_meson_g12a_mipi_dphy_init(struct phy *phy)
{
	struct phy_meson_g12a_mipi_dphy_priv *priv = phy_get_drvdata(phy);
	int ret;
...
static int phy_meson_g12a_mipi_dphy_configure(struct phy *phy,
...
static int phy_meson_g12a_mipi_dphy_power_on(struct phy *phy)
{
	struct phy_meson_g12a_mipi_dphy_priv *priv = phy_get_drvdata(phy);
	int ret;
	...
	regmap_write(priv->regmap, MIPI_DSI_CLK_TIM,
		     priv->config.clk_trail |
		     ((priv->config.clk_post + priv->config.hs_trail) << 8) |
		     (priv->config.clk_zero << 16) |
		     (priv->config.clk_prepare << 24));
	regmap_write(priv->regmap, MIPI_DSI_CLK_TIM1, priv->config.clk_pre);

	if (priv->config.hs_clk_rate > 500000000UL)
		regmap_write(priv->regmap, MIPI_DSI_HS_TIM,
			     priv->config.hs_exit |
			     (priv->config.hs_trail << 8) |
			     (priv->config.hs_zero << 16) |
			     (priv->config.hs_prepare << 24));
	else
		regmap_write(priv->regmap, MIPI_DSI_HS_TIM,
			     priv->config.hs_exit |
			     ((priv->config.hs_trail / 2) << 8) |
			     (priv->config.hs_zero << 16) |
			     (priv->config.hs_prepare << 24));

	regmap_write(priv->regmap, MIPI_DSI_LP_TIM,
		     priv->config.lpx |
		     (priv->config.ta_sure << 8) |
		     (priv->config.ta_go << 16) |
		     (priv->config.ta_get << 24));

	regmap_write(priv->regmap, MIPI_DSI_ANA_UP_TIM, 0x0100);
	regmap_write(priv->regmap, MIPI_DSI_INIT_TIM, priv->config.init);
	regmap_write(priv->regmap, MIPI_DSI_WAKEUP_TIM, priv->config.wakeup);
...
static int phy_meson_g12a_mipi_dphy_power_off(struct phy *phy)
{
	struct phy_meson_g12a_mipi_dphy_priv *priv = phy_get_drvdata(phy);
...
static int phy_meson_g12a_mipi_dphy_exit(struct phy *phy)
{
	struct phy_meson_g12a_mipi_dphy_priv *priv = phy_get_drvdata(phy);
	int ret;
...
static const struct phy_ops phy_meson_g12a_mipi_dphy_ops = {
	.configure	= phy_meson_g12a_mipi_dphy_configure,
	.init		= phy_meson_g12a_mipi_dphy_init,
	.exit		= phy_meson_g12a_mipi_dphy_exit,
	.power_on	= phy_meson_g12a_mipi_dphy_power_on,
	.power_off	= phy_meson_g12a_mipi_dphy_power_off,
	.owner		= THIS_MODULE,
};

static int phy_meson_g12a_mipi_dphy_probe(struct platform_device *pdev)
{
	struct device *dev = &pdev->dev;
	struct phy_provider *phy_provider;
	struct resource *res;
	struct phy_meson_g12a_mipi_dphy_priv *priv;
	struct phy *phy;
	void __iomem *base;
	int ret;
	...
	priv->regmap = devm_regmap_init_mmio(dev, base,
					&phy_meson_g12a_mipi_dphy_regmap_conf);
	...
	phy = devm_phy_create(dev, NULL, &phy_meson_g12a_mipi_dphy_ops);
	if (IS_ERR(phy)) {
		ret = PTR_ERR(phy);
		if (ret != -EPROBE_DEFER)
...
static const struct of_device_id phy_meson_g12a_mipi_dphy_of_match[] = {
	{ .compatible = "amlogic,g12a-mipi-dphy", },
	{ },
};
MODULE_DEVICE_TABLE(of, phy_meson_g12a_mipi_dphy_of_match);

static struct platform_driver phy_meson_g12a_mipi_dphy_driver = {
	.probe	= phy_meson_g12a_mipi_dphy_probe,
	.driver	= {
		.name		= "phy-meson-g12a-mipi-dphy",
		.of_match_table	= phy_meson_g12a_mipi_dphy_of_match,
	},
};
module_platform_driver(phy_meson_g12a_mipi_dphy_driver);

MODULE_AUTHOR("Neil Armstrong <narmstrong@baylibre.com>");
MODULE_DESCRIPTION("Meson G12A MIPI DPHY driver");
