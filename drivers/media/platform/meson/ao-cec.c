...
static void meson_ao_cec_irq_tx(struct meson_ao_cec_device *ao_cec)
{
	...
	if (tx_status == CEC_TX_STATUS_OK || !ao_cec->retries) {
		/* Stop TX */
		meson_ao_cec_write(ao_cec, CEC_TX_MSG_CMD, TX_NO_OP);

		cec_transmit_done(ao_cec->adap, tx_status, 0, 0, 0, 0);
	} else {
		ao_cec->retries--;
...
static int meson_ao_cec_set_log_addr(struct cec_adapter *adap, u8 logical_addr)
{
	...
	meson_ao_cec_write(ao_cec, CEC_LOGICAL_ADDR0, 0);
...
static int meson_ao_cec_probe(struct platform_device *pdev)
{
	...
	np = of_parse_phandle(pdev->dev.of_node, "hdmi-controller", 0);
	if (!np) {
