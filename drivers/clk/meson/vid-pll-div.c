 ...
 *
 * Here are provided the commonly used fraction values.
 */

#define VID_PLL_BYPASS		BIT(18)
#define VID_PLL_VAL_MASK	0x7fff
#define VID_PLL_SEL_SHIFT	16
#define VID_PLL_SEL_MASK	0x3
...
{
	struct meson_vid_pll_div *vid_pll = to_meson_vid_pll_div(hw);
	const struct vid_pll_div *div;
	unsigned int shift_val;
	unsigned int shift_sel;
	u32 reg;

	reg = readl(vid_pll->base);

	if (reg & VID_PLL_BYPASS)
		return parent_rate;

	shift_val = reg & VID_PLL_VAL_MASK;
	shift_sel = (reg >> VID_PLL_SEL_SHIFT) & VID_PLL_SEL_MASK;

	div = _get_table_val(shift_val, shift_sel);
	if (!div || !div->frac_top) {
		pr_info("%s: Invalid rate for vid_pll_div\n", __func__);
		return parent_rate;
	}
