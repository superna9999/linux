					       ...
					       unsigned long parent_rate)
{
	...
	if (reg0 & CLK_CNTL0_DUALDIV_EN) {
		...
		pr_info("%s:%d n %ld %ld m %ld %ld f %ld %ld p %ld %ld\n",
			__func__, __LINE__, n1, n2, m1, m2, f1, f2, p1, p2);
				     ...
				     unsigned long *prate)
{
	...
	pr_info("%s:%d %ld %ld %p\n", __func__, __LINE__, rate, *prate, freq);
				  ...
				  unsigned long parent_rate)
{
	...
	pr_info("%s:%d %ld %ld %p\n", __func__, __LINE__, rate, parent_rate, freq);
