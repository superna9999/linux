/*
 * AmLogic S905 X/D / GXL Clock Controller Driver
 *
...
static int gxl_clkc_probe(struct platform_device *pdev)
{
	...
	for (clkid = 0; clkid < NR_CLKS; clkid++) {
		ret = devm_clk_hw_register(dev, gxl_hw_onecell_data.hws[clkid]);
		if (ret)
			goto iounmap;
	}
	 ...
	 */
	parent_hw = clk_hw_get_parent(&gxl_cpu_clk.hw);
...
static const struct of_device_id gxl_clkc_match_table[] = {
	{ .compatible = "amlogic,gxl-clkc" },
	{ }
};

static struct platform_driver gxl_driver = {
	.probe		= gxl_clkc_probe,
	.driver		= {
		.name	= "gxl-clkc",
		.of_match_table = gxl_clkc_match_table,
	},
};

builtin_platform_driver(gxl_driver);
