			      ...
			      int *m, bool csc_on)
{
	if (m_select == VIU_MATRIX_OSD) {
		...
		writel_bits_relaxed(1, csc_on ? 1 : 0,
			priv->io_base + _REG(VIU_OSD1_MATRIX_COLMOD_COEF42));
		writel_bits_relaxed(1, 0,
			priv->io_base + _REG(VIU_OSD1_MATRIX_COLMOD_COEF42));
		...
		writel_bits_relaxed(1 << 30, csc_on ? 1 << 30 : 0,
			priv->io_base + _REG(VIU_OSD1_EOTF_CTL));
		writel_bits_relaxed(1 << 31, csc_on ? 1 << 31 : 0,
			priv->io_base + _REG(VIU_OSD1_EOTF_CTL));
	}
}
