...
static int dw_mipi_dsi_phy_init(void *priv_data)
{
	...
	pr_info("%s: px_clk_rate before %d\n", __func__, clk_get_rate(mipi_dsi->px_clk));

	ret = clk_set_min_rate(mipi_dsi->px_clk, mipi_dsi->mode->clock * 1000);
	if (ret) {
		dev_err(mipi_dsi->dev, "Failed to set DSI Pixel clock rate %lu (%d)\n",
		        mipi_dsi->mode->clock * 1000, ret);
		return ret;
	}

	pr_info("%s: px_clk_rate after %d\n", __func__, clk_get_rate(mipi_dsi->px_clk));
			  ...
			  unsigned int *lane_mbps)
{
	...
	pr_info("%s: bit_clk_rate target %d\n", __func__, mipi_dsi->phy_opts.mipi_dphy.hs_clk_rate); // + (mode->clock * 1000));
	pr_info("%s: bit_clk_rate before %d\n", __func__, clk_get_rate(mipi_dsi->bit_clk));

	/* Try to set a rate at min hs_clk_rate + pclk */
	ret = clk_set_rate(mipi_dsi->bit_clk,
			       mipi_dsi->phy_opts.mipi_dphy.hs_clk_rate); // + (mode->clock * 1000));
	if (ret) {
		dev_err(mipi_dsi->dev, "Failed to set DSI Bit clock rate %lu (ret)\n",
			mipi_dsi->phy_opts.mipi_dphy.hs_clk_rate, ret); // + (mode->clock * 1000), ret);
		return ret;
	}

	pr_info("%s: bit_clk_rate after %d\n", __func__, clk_get_rate(mipi_dsi->bit_clk));
...
static int meson_dw_mipi_dsi_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(mipi_dsi->phy))
		return dev_err_probe(&pdev->dev, PTR_ERR(mipi_dsi->phy),
				     "failed to get mipi dphy\n");

	mipi_dsi->bit_clk = devm_clk_get_enabled(&pdev->dev, "bit_clk");
	if (IS_ERR(mipi_dsi->bit_clk))
		return dev_err_probe(&pdev->dev, PTR_ERR(mipi_dsi->bit_clk),
				     "Unable to get PLL clk\n");

	mipi_dsi->px_clk = devm_clk_get_enabled(&pdev->dev, "px_clk");
	if (IS_ERR(mipi_dsi->px_clk))
		return dev_err_probe(&pdev->dev, PTR_ERR(mipi_dsi->px_clk),
				     "Unable to get PLL clk\n");
